...
interface EditAhspModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: RabData;
  onDataUpdate: (newData: RabData) => void;
  editingAhsCode: string | null;
}
...
export const EditAhspModal: React.FC<EditAhspModalProps> = ({
  isOpen,
  onClose,
  data,
  onDataUpdate,
  ...
  // Initialize values when the edit code changes
  useEffect(() => {
    if (isOpen && editingAhsCode) {
      const src = data.ahsp.find(a => a.code === editingAhsCode);
      if (src) {
        setNewAhsCode(src.code);
        setNewAhsName(src.name);
        setNewAhsUnit(src.unit);
        setNewAhsOverhead(src.overheadPercent);
        const components = [
          ...src.labor.map(l => ({ type: 'labor' as const, code: l.code, name: l.type || l.code, unit: l.unit, rate: l.rate, coefficient: l.coefficient })),
          ...src.materials.map(m => ({ type: 'material' as const, code: m.code, name: m.name, unit: m.unit, rate: m.rate, coefficient: m.coefficient })),
        ];
        setNewAhsComponents(components);
      }
    }
  }, [isOpen, editingAhsCode, data.ahsp]);

  if (!isOpen || !editingAhsCode) return null;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newAhsCode || !newAhsName || !newAhsUnit || !editingAhsCode) return;
    ...
    const updatedAhs = {
      ...
      labor,
      materials,
      equipment: data.ahsp.find(a => a.code === editingAhsCode)?.equipment || [],
    ...
    onDataUpdate(recalculateRab({
      ...data,
      ahsp: data.ahsp.map(a => a.code === editingAhsCode ? updatedAhs : a)
    }));
    ...
    <div className="fixed inset-0 bg-slate-950/65 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="saas-modal p-6 max-w-xl w-full space-y-4 max-h-[90vh] overflow-y-auto">
        <h4 className="text-base font-extrabold text-slate-800 flex items-center gap-2">
          <Edit3 className="h-5 w-5 text-blue-500" /> Edit Formula Analisa
        </h4>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1">
              ...
              <input 
                ...
                disabled 
                value={newAhsCode} 
                className="saas-input font-mono bg-slate-100 cursor-not-allowed" 
              />
            </div>
